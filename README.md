...
**Текущий релиз:** v1.7.0 — финальная стабилизация фазы v1.x
...
1. **Выбор планировки** — 3 типовых плана с 2D-превью, окнами и дверями
2. **Черновая отделка** — стяжка (включая тёплый пол), электрика (BFS-маршрутизация), штукатурка, потолок
3. **Чистовая отделка** — покрытие пола (ламинат / плитка / кварц-винил с паттернами), стены, расстановка мебели из каталога с drag-to-move и отзеркаливанием
4. **Итоги** — двухуровневая группированная смета с min-max ценами, экспорт PDF / PNG / JSON

### Внутренние инструменты

- Секретный редактор планировок: `/?editor=1&token=<TOKEN>` (TOKEN из `VITE_LAYOUT_EDITOR_TOKEN`)
